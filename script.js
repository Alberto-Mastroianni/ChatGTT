...
// Gestione dell'alert disclaimer
function showDisclaimerAlert() {
    const alert = document.getElementById('disclaimerAlert');
    const closeBtn = document.getElementById('disclaimerClose');
    
    // Mostra l'alert dopo 30 secondi
    setTimeout(() => {
        alert.classList.add('show');
    }, 30000);
    
    // Chiudi l'alert quando si clicca sulla X
    closeBtn.addEventListener('click', () => {
        alert.classList.remove('show');
    });
    
    // Chiudi l'alert automaticamente dopo 15 secondi dalla comparsa
    setTimeout(() => {
        alert.classList.remove('show');
    }, 45000);
}

// Avvia la gestione dell'alert al caricamento della pagina
window.addEventListener('DOMContentLoaded', showDisclaimerAlert);